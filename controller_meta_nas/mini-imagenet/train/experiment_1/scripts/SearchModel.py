    ...
    def forward(self, s0, s1, weights, params=None, cell_idx=None):
        ...
        for i in range(self._steps):
            ...
            for j, s in enumerate(states):
                ...
                temp.append(self._ops[offset + j](s, weights[offset + j], params_ops[str(offset+j)], cell_idx=cell_idx, mix_idx=offset+j))
            new_s = sum(temp)
            offset += len(states)
            states.append(new_s)
...
class Network(nn.Module):
    ...
    def _load_pretrained_alphas(self):
        self.alphas_normal = torch.load(self._args.pretrained_model)['state_dict_theta'][0].to(self.device)  # self.alphas_normal.shape = [14, 8]
        self.alphas_normal.requires_grad = True
