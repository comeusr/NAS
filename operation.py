...
OPS = {
    ...
    'avg_pool_3x3': lambda C, stride, affine, device, args: nn.AvgPool2d(3, stride=stride, padding=1, count_include_pad=False),
    'max_pool_3x3': lambda C, stride, affine, device, args: nn.MaxPool2d(3, stride=stride, padding=1),
    'skip_connect': lambda C, stride, affine, device, args: Identity() if stride == 1 else FactorizedReduce(C, C, device, args,affine=affine),
    'sep_conv_3x3': lambda C, stride, affine, device, args: SepConv(C, C, 3, stride, 1, device, args, affine=affine),
    'sep_conv_5x5': lambda C, stride, affine, device, args: SepConv(C, C, 5, stride, 2, device, args, affine=affine),
    'sep_conv_7x7': lambda C, stride, affine, device, args: SepConv(C, C, 7, stride, 3, device, args, affine=affine),
    'dil_conv_3x3': lambda C, stride, affine, device, args: DilConv(C, C, 3, stride, 2, 2, device, args, affine=affine),
    'dil_conv_5x5': lambda C, stride, affine, device, args: DilConv(C, C, 5, stride, 4, 2, device, args, affine=affine),
    'conv_7x1_1x7': lambda C, stride, affine, device, args: Conv_7x1_1x7(C, stride, affine),
}
...
class ReLUConvBN(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, device, args, affine=False):
        super().__init__()

        self.affine = affine
        self.conv = MetaConv2dLayer(C_in, C_out, kernel_size, stride=stride, padding=padding, use_bias=False)
        self.bn = MetaBatchNormLayer(C_out, device, args, affine=affine)
        self.relu = nn.ReLU(inplace=False)
    ...
    def forward(self, x, step, params=None, training=True, backup_running_statistics=False):
        param_bn = None

        if params is not None:
            params = extract_top_level_dict(current_dict=params)
            param_conv = params['conv']
            if self.affine:
                param_bn = params['bn']

        out = self.relu(x)
        out = self.conv(out, param_conv)
        out = self.bn(out, num_step=step, params=param_bn, training=training,
...
class DilConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, device, args, affine=True):
        super().__init__()
        self.affine = affine
        ...
        self.bn = MetaBatchNormLayer(C_out, device, args, affine=affine)  # nn.BatchNorm2d(C_out, affine=affine)
    ...
    def forward(self, x, step, params, training=True, backup_running_statistics=False):
        param_bn = None
        ...
        out = self.relu(x)
        out = self.conv1(out, params=param_conv1)
        out = self.conv2(out, params=param_conv2)
        out = self.bn(out, num_step=step, params=param_bn, training=training,
                      backup_running_statistics=backup_running_statistics)
        return out
...
class SepConv(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, device, args, affine=True):
        super().__init__()
        ...
        self.bn1 = MetaBatchNormLayer(C_in, device, args, affine=affine)#nn.BatchNorm2d(C_in, affine=affine)
        ...
        self.bn2 = MetaBatchNormLayer(C_out, device, args, affine=affine)#nn.BatchNorm2d(C_out, affine=affine)
    ...
    def forward(self, x, step, params, training=True, backup_running_statistics=False):
        ...
        out = self.bn1(out, num_step=step, params=bn1_params, training=training,
                       backup_running_statistics=backup_running_statistics)
        out = self.conv3(out, conv3_params)
        out = self.conv4(out, conv4_params)
        out = self.bn2(out, num_step=step, params=bn2_params, training=training,
                       backup_running_statistics=backup_running_statistics)
        return out
...
class FactorizedReduce(nn.Module):
    ...
    def __init__(self, C_in, C_out, device, args, affine=True):
        super().__init__()
        self.affine = affine
        assert C_out % 2 == 0
        self.relu = nn.ReLU(inplace=False)
